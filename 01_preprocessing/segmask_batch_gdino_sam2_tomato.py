...
class ImageSegmenter():
    ...
    def apply(self, 
        ...
        if not sam2_track:
            ...
            if merged_bbox is None:
                ...
                cleaned_mask = self.get_exg_otsu_mask(image_np, fill_ratio=hole_fill_ratio, remove_ratio=noise_remove_ratio)
                ...
                self.logger.info(f"   => use psp method {np_bbox}!!!")
                ...
                # cleaned_mask_in_bbox = self.get_exg_otsu_mask(image_np_in_bbox, fill_ratio=hole_fill_ratio, remove_ratio=noise_remove_ratio)
                ...
                cleaned_mask_in_bbox = self.get_psp_mask(img_np_in_bbox, )
    ...
    @staticmethod
    def get_exg_otsu_mask(img_np, fill_ratio=0.001, remove_ratio=0.001):
        h, w, d = img_np.shape
        # 1. 预处理：转为浮点数 (0-1范围)，避免计算时溢出
        # 这一步很重要，因为 2*G - R - B 计算过程中会出现负数
        ...
        try:
            thresh_exg = threshold_otsu(exg_image)
            # 有时候 Otsu 算出的阈值太低(比如全黑背景)，可以加一个最小值限制
            if thresh_exg < 0: thresh_exg = 0 
            mask_green = exg_image > thresh_exg
        except Exception:
            # 如果图像完全是黑的或单色，otsu会报错
        ...
        # ---------------------------------------------------------
        # 填充闭合区域内部的小黑洞
        filled_mask = ski.morphology.remove_small_holes(mask_green, area_threshold=h*w*fill_ratio)
        # 去除散乱的小白点噪声
        cleaned_mask = ski.morphology.remove_small_objects(filled_mask, min_size=h*w*remove_ratio)
        return cleaned_mask
    
    @staticmethod
    def get_psp_mask(img_np, mask):
        return psp_refiner.refine(img_np, mask, fast=False, L=900)
...
if __name__ == "__main__":
    working_directory = r"/home/crest/w/hwang_Pro/data/202511_kubota_tomato"

    psp_model_path = Path(__file__).parent / 'checkpoints' / 'psp_models' / 'cascade_psp'
    ...
    SAM_TRACK_IMG_NUM = 2
    RANDOM_SAVE_POSSIBILITY = 1  # save 10% of results for preview

    logger.info(":: init ImageSegmentor")
    imgseg = ImageSegmenter(detect_object="plant", logger=logger)
    ...
    for foldername, subfolders, filenames in os.walk(img_folder):
        ...
        for filename in filenames:
            ...
            cv_mask = (result.masks.data.cpu().numpy().squeeze() * 255).astype(np.uint8)
            img_np = (result.orig_img.data.cpu().numpy().squeeze() * 255).astype(np.uint8)

            

            # ski.io.imsave(mask_path, cv_mask)
            ski.io.imsave(mask_path, psp_mask)
